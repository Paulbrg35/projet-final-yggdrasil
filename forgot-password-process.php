...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!validateCSRFToken($csrf_token)) {
            error_log("Session expirée : token invalide");
            $_SESSION['reset_error'] = "Session expirée. Veuillez recharger la page.";
        ...
        if ($ip_attempts >= 5) {
            error_log("Rate limit IP dépassé pour $ip");
            throw new Exception('Trop de tentatives depuis cette adresse IP.');
        }
        ...
        if ($email_attempts >= 3) {
            ...
            throw new Exception('Trop de tentatives pour cette adresse email.');
        }
